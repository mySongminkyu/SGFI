...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(self, image_embeddings, image_pe, sparse_prompt_embeddings,
        ...
        # Step 1: upsample 4x -> 2x
        step1 = dc1(src)
        if has_hr:
            if self.injector_s1 is not None:
                step1, _ = self.injector_s1(low_res=step1, high_res=feat_s1)
            else:
                step1 = step1 + feat_s1
        up_after1 = act1(ln1(step1))

        # Step 2: upsample 2x -> 1x
        step2 = dc2(up_after1)
        if has_hr:
            if self.injector_s2 is not None:
                step2, _ = self.injector_s2(low_res=step2, high_res=feat_s0)
            else:
                step2 = step2 + feat_s0
